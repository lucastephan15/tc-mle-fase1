...
| 2026-08-25 | 1 / 11 | **ML Canvas escrito** (`docs/ML_CANVAS.md`) — tarefa nomeada da Etapa 1 do enunciado, cumprida com o conteúdo que já existia | não é retrabalho: é a §0 deste log **reformatada para leitura de negócio**, com os números que só a execução produziu (recall@10% 28,6% · custo −54,7% · disparidade 58,89 pp). O que ele acrescenta de novo são dois blocos que nenhum outro documento tinha: **stakeholders com a coluna "o que decide"** — e o RACI declarado como acumulado, nomeando as duas decisões que **exigiriam dono que este projeto não tem** — e os **cinco pressupostos não validados**, com a consequência escrita (*se qualquer um for falso, o que muda é o enquadramento, e o caminho de volta é a Etapa 0*) |
| 2026-08-25 | 1 | **`notebooks/01_eda.ipynb`** passa a existir — entregável nomeado pelo enunciado, gerado do `.py` por `make notebook` | a fonte versionada continua sendo o **`.py` em formato percent** (`# %%`): JSON de notebook é uma linha só, **inmergeável**, e muda de conteúdo a cada abertura. O `.ipynb` é derivado e executado (12 células de código, **12 com saída, 3 figuras, 0 erros** — verificado por script, não por inspeção). 🔑 *A regra da casa não mudou:* nenhuma linha de `notebooks/` é importada pelo pipeline. O que mudou é que a exploração passou a ser **legível** — 20 células de markdown ligando cada achado à etapa que ele decidiu |
| 2026-08-25 | 1 | 🚨 **Armadilha encontrada ao converter: `plt.show()` como script BLOQUEIA o processo** | com o backend interativo do macOS, rodar o `.py` espera a janela fechar — o processo trava **sem erro, sem mensagem e sem timeout** (aconteceu, e só o `pkill` resolveu). Dentro do kernel do Jupyter o inline já está ativo e trocá-lo apagaria as figuras. ✅ `if "ipykernel" not in sys.modules: matplotlib.use("Agg")` distingue os dois casos sem depender de variável de ambiente — *o mesmo arquivo tem de rodar em dois contextos, e o contexto é detectável* |
| 2026-08-25 | 9.5 | **4 ferramentas de autoria do notebook no `requirements.in`** (jupytext, nbclient, nbconvert, ipykernel) — e a decisão de **não** criar um quarto arquivo de dependências | lock regenerado por `pip freeze` do venv já instalado: **45 dists novas e ZERO versões movidas** (verificado por diff). O que precisava ser isolado — o runtime da API — já está em `requirements-serve.in`, que continua com 30 dists e sem nada disto. Um `requirements-notebook.txt` seria mais um arquivo a manter em sincronia para poupar 45 dists leves de um CI que já instala `torch` (529 MB) com cache. Mesmo estatuto do `ruff` e do `pytest`. ⚠️ `nbconvert` é dependência **não declarada** do `jupytext --execute`: sem ela o comando falha pedindo "nbconvert e ipykernel", e só aparece na primeira execução |

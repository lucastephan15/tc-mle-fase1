...
| 33 | Execução 9.5 | ⚠️ **Push pendente** — o CI só é real quando roda no GitHub. Enquanto não houver push, o badge do README aponta para um workflow que nunca executou. Conferir também o **tempo do run**: `requirements.txt` inclui `torch`, e se passar de poucos minutos vale separar um `requirements-ci` (CI lento é CI que o time desliga) | 9.5 | ⬜ |
